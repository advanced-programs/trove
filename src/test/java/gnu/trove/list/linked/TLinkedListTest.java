...
public class TLinkedListTest extends TestCase {
    ...
    public void testNextIterator() throws Exception {
        ...
        int count = 0;
		for (Iterator<Data> i = list.iterator(); i.hasNext();) {
            assertEquals( data[count++], i.next() );
        }
        assertEquals( data.length, count );

        count = 4;
		for (Iterator<Data> i = list.listIterator(4); i.hasNext();) {
            assertEquals( data[count++], i.next() );
        }
        assertEquals( data.length, count );
    }
    ...
    public void testPreviousIterator() throws Exception {
        ...
        int count = 100;
		for (ListIterator<Data> i = list.listIterator(list.size());
              i.hasPrevious(); ) {
        ...
        count = 5;
		for (ListIterator<Data> i = list.listIterator(count); i.hasPrevious();) {
            assertEquals( data[--count], i.previous() );
        }
        assertEquals( 0, count );
    }
